...
public class FleeDemo extends ScreensaverBase {
	...
	public void init() {
		...
		boid = new Orb(mass, initPos.get(1), initVelo, radius, maxForce,
				maxSpeed);
	}
	...
	public void render(Graphics2D g2) {
		...
		fillAt(g2, circle, boid.position().x, boid.position().y);

		// check for collision with each other
		if (intersecting(ball, boid)) {
			Vector3D[] result = Physics.collide(ball.mass(), ball.position(),
					ball.velocity(), boid.mass(), boid.position(), boid
							.velocity());

			ball.setVelocity(result[0]);
			boid.setVelocity(result[1]);
		...
		collideWithWall(boid);

		// update position
		ball.move();
		
		// steer and update boid
		if(V3.distance(ball.position(), boid.position()) < 200) {
			Vector3D steering = Steering.flee(boid, ball.position(), 0.7);
			boid.steer(steering);
		}
		boid.move();
		boid.rebound -= 1;
