...
public class Arrival implements Behavior {
	...
	public Arrival(Locomotion boid, Vector2D position) {
		this.boid = boid;
		this.position = position;
		this.coeff = 1.0; 
	}
	
	public Arrival(Locomotion boid, Vector2D position, double coeff) {
		this.boid = boid;
		this.position = position;
		this.coeff = coeff;
	}
	...
	public Vector2D steeringForce() {
		return Steering.arrive(boid, position, coeff);
	}
